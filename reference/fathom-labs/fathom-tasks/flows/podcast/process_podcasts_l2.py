...
def fetch_podcast_ids_to_update():
    ...
    level_2_id_groups = core.utility.balanced_chunks(level_2_ids, 4)
    
    formatted_level_2_id_groups = []
    for level_2_id_group in level_2_id_groups:
        formatted_level_2_id_group = {
            'ids': level_2_id_group
        }
        formatted_level_2_id_groups.append(formatted_level_2_id_group)

    return formatted_level_2_id_groups

@task(timeout = 32768)
def process_podcasts(id_group):
    for id in id_group['ids']:
        try:
            with transaction("process_podcast_l2"):
...
flow.run_config = LocalRun(working_dir="/app", labels=["fathom-podcast-processing-l2"])
