<template>
  <div class="upload-file-cards">
    ...
    <div 
      ...
      :key="i"
    >
      <NuxtLink  :to="`/projects/${project.id}`" :class="['upload-file-card__podcast-image-wrapper ml-3 rounded-lg', highlightedProject === i ? 'bg-gray-50' : 'bg-gray-100']">
        <svg v-if="!project.image_url || project.image_url == 'null'" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M3 7V17C3 18.1046 3.89543 19 5 19H19C20.1046 19 21 18.1046 21 17V9C21 7.89543 20.1046 7 19 7H13L11 5H5C3.89543 5 3 5.89543 3 7Z" stroke="#6B7280" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
        </svg>
        <img class="rounded-lg w-full h-full object-cover  " v-if="project.image_url && project.image_url != 'null'" :src="project.image_url">

      </NuxtLink>
      
      <div @click="handleViewProject(project.id)" class="upload-file-card__project-information">
        <NuxtLink :to="`/projects/${project.id}`" class="upload-file-card__project-name">{{ project.name  }}</NuxtLink>
...
.upload-file-card {
  ...
  &__project {
    ...
    &-name {
      @apply text-gray-900 text-sm leading-5 font-medium mb-2 mt-0;
    }
    &-actions {
      @apply px-3;
    }
