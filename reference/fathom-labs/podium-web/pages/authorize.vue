<template>
  <div class="flex h-screen">
    <div class="m-auto" style="transform: scale(2.0);">
...
const podiumToken = useCookie('podium_token', {'maxAge': (60 * 60 * 24 * 365) / 2})

if (!podiumToken || !podiumToken.value) {
  podiumToken.value = token.value.accessToken
  navigateTo('/authorize')
}

await mainStore.retrieveUser()

const directUser = (defaultRoute) => {
  if  (pendingStripePriceId.value && pendingStripeQuantity.value) {
    const query = {
      stripePriceId: pendingStripePriceId.value,
      stripeQuantity: pendingStripeQuantity.value
    }
    ...
    navigateTo({ path: '/begin-purchase', query: query })
  } else {
    const isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)

    const encodedDefaultRoute = encodeURIComponent(defaultRoute)
    if(isMobile) {
      navigateTo(`/mobile?defaultRoute=${encodedDefaultRoute}`)
    } else {
...
onMounted(async () => {
  ...
  if (mainStore.user) {
    ...
    if (firstTimeVisit.value) {
      ...
      if (mainStore.user.new_user_info_complete) {
        directUser('/dashboard')

      } else {
        directUser('/new-user-info')
      }
    }
  } else {
    navigateTo('/login')
