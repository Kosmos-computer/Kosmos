<template>
  <div class="flex h-screen">
    <div class="mx-auto pt-20">
      <div class="">
        <svg  class="mx-auto mb-24 lg:scale-400 scale-200" width="112" height="24" viewBox="0 0 112 24" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <div v-if="!completed" class="h-52">
        <div class="text-center font-bold text-3xl mb-10 ">
          Get 4 Hours of Audio Processing FREE!
        </div>
        ...
        <div class="text-center mb-10">
          <button @click="submit()" class="w-80 bg-black text-white hover:bg-white border-3 border-white hover:border-black hover:text-black text-xl font-semibold rounded-lg py-3 px-4">
                Get it!
          </button>
        </div>
      </div>
...
import podiumExpo from "~/apollo/mutations/podiumExpo.gql";
...
const submit = async () => {
  ...
  try {
    const { mutate, loading } = useMutation(podiumExpo, {
      variables: {
        email: email.value
      }
    });
    ...
    console.log(data)
    if (data && data.podiumExpo.ok) {
      completed.value = true
    } else {
      alert('Bonus credits have already been granted!')
    }
  } catch (e) {
    console.log(e)
    alert('There was an API error. Please try again.')
  }

  //reset the form in 10 seconds
  setTimeout(() => {
    email.value = ''
